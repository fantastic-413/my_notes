...
- 但在给定点 $\boldsymbol{\theta}=\boldsymbol{\theta}^0$ 的前提下，可以利用泰勒展开，近似表示出 Loss 函数在 $\boldsymbol{\theta}^0$ 附近的函数形式。
...
- 那么我们考率使用**红色方框中的项**来协助判断临界点的类型。<img src="img/image-20221019201016984.png" alt="image-20221019201016984" style="zoom:33%;" />
...
3、判断好临界点类型后，如果卡住的临界点是鞍点，矩阵 $\boldsymbol{H}$ 可以指明参数更新的方向。

- 我们期望向 Loss 值变小的方向更新参数，也就是要使 $\boldsymbol{L}(\boldsymbol{\theta}^1)<\boldsymbol{L}(\boldsymbol{\theta}^0)$ ，即：

$$
\boldsymbol{v}^T\boldsymbol{H}\boldsymbol{v}<0
$$

- 我们计算矩阵 $\boldsymbol{H}$ 的负特征值相关的特征向量 $\boldsymbol{u}$，依据 $\boldsymbol{H}\boldsymbol{u}=\lambda\boldsymbol{u}$ 不难得到：

$$
\boldsymbol{u}^T\boldsymbol{H}\boldsymbol{u}=\boldsymbol{u}^T\lambda\boldsymbol{u}=\lambda\|\boldsymbol{u}\|^2<0 \space (\lambda<0)
$$

- 那么只需要使 $\boldsymbol{v}=\boldsymbol{u}$，也即：

$$
\boldsymbol{\theta}-\boldsymbol{\theta}'=\boldsymbol{u} \\
\boldsymbol{\theta}=\boldsymbol{\theta}'+\boldsymbol{u}
$$

- 使用 $\boldsymbol{\theta}=\boldsymbol{\theta}'+\boldsymbol{u}$ 来更新 $\boldsymbol{\theta}$ 即可。
...
- 实际情况图如下：所有卡在临界点的模型的 minimum radio 与 Loss 值的关系图。
